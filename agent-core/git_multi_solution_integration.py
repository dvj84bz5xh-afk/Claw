...
# 导入Git上下文模块
from git_context_integration import GitContextProvider, GitContextUncertaintyAnalyzer, GitStatus
...
class GitSolutionStrategySelector:
    ...
    def __init__(self, config: GitSolutionIntegrationConfig | None = None):
...
class GitEnhancedSolutionEvaluator:
    ...
    def evaluate_with_git_context(
        self, solution: SolutionCandidate, git_status: GitStatus | None = None
        ...
        enhanced.implementation_risk_score = self._calculate_implementation_risk(
            solution, git_status
        )
        enhanced.rollback_complexity_score = self._calculate_rollback_complexity(
            solution, git_status
        )
        enhanced.branch_strategy_fit_score = self._calculate_branch_strategy_fit(
            solution, git_status
        )

        # 生成Git工作流程建议
        enhanced.recommended_git_workflow = self._generate_git_workflow_recommendation(
            solution, git_status
        )
    ...
    def _calculate_git_compatibility(
        self, solution: SolutionCandidate, git_status: GitStatus
    ) -> float:
        """计算Git兼容性评分"""
        score = 0.8  # 基础分数
        ...
        if (
            ...
            and solution.resource_intensity == ResourceIntensity.HEAVY
        ):
            score -= 0.2  # 重型方案在有未提交修改时兼容性降低

        return max(0.0, min(1.0, score))

    def _calculate_implementation_risk(
        self, solution: SolutionCandidate, git_status: GitStatus
    ) -> float:
        """计算实施风险评分"""
        base_risk = {
    ...
    def _calculate_rollback_complexity(
        self, solution: SolutionCandidate, git_status: GitStatus
    ) -> float:
        """计算回滚复杂度评分"""
    ...
    def _calculate_branch_strategy_fit(
        self, solution: SolutionCandidate, git_status: GitStatus
    ) -> float:
        """计算分支策略匹配度评分"""
        fit_score = 0.7
    ...
    def _generate_git_workflow_recommendation(
        self, solution: SolutionCandidate, git_status: GitStatus
    ) -> str:
        """生成Git工作流程建议"""
        recommendations = []
...
class GitContextAwareSolutionGenerator:
    ...
    def __init__(
        ...
        solution_generator: MultiSolutionGenerator | None = None,
        config: GitSolutionIntegrationConfig | None = None,
    ):
    ...
    def generate_solutions_for_context(
        self, problem: str, context: dict[str, Any] | None = None, num_solutions: int | None = None
    ) -> list[GitEnhancedSolutionCandidate]:
        """基于Git上下文生成增强的解决方案"""

        # 获取当前Git状态
        git_status = self.git_context.get_detailed_status()
        print(
            f"[Git上下文] 当前分支: {git_status.current_branch}, 变更文件: {len(git_status.modified_files or []) + len(git_status.untracked_files or [])}"
        )
    ...
    def _prepare_generation_params(
        self, strategy: GitSolutionStrategy, git_status: GitStatus, context: dict[str, Any] | None
    ) -> dict[str, Any]:
        """准备生成参数"""
        params = {
    ...
    def generate_comparison_report(
        self, solutions: list[GitEnhancedSolutionCandidate], include_git_analysis: bool = True
    ) -> str:
        """生成解决方案比较报告"""
        report = "# 解决方案比较报告\n\n"
        ...
        # 方案比较表格
        report += "## 方案排名\n\n"
        report += "| 排名 | 方案ID | 总分 | Git兼容分 | 实施风险 | 类型 | 推荐工作流程 |\n"
        report += "|------|--------|------|-----------|----------|------|--------------|\n"
...
def main():
    ...
    for problem in test_problems:
        ...
        if solutions:
            print(
                f"最佳方案: {solutions[0].id[:8]} (评分: {solutions[0].total_score_with_git:.2f})"
            )
