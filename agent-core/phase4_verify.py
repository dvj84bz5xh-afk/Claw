...
def test_mock_services():
    ...
    try:
        from mock_services import TestHarness
        ...
        harness.add_test(
            name="Bash command", tool="bash", arguments={"command": "ls -la"}, expected_success=True
        )
        ...
        if result["passed"] > 0:
            ...
            print("  [WARN] Mock Services: No tests passed")
            return False
...
def test_error_recovery():
    ...
    try:
        ...
        record = manager.record_error(
            error=ConnectionError("Test error"), component="test", severity=ErrorSeverity.MEDIUM
        )
        ...
        print(
            f"  [OK] Error Recovery: {summary['total']} errors, {summary['recovery_rate'] * 100:.0f}% recovery rate"
        )
        return True
...
def test_audit_logger():
    ...
    try:
        from audit_logger import AuditCategory, AuditLogger
...
def test_team_registry():
    ...
    try:
        from team_registry import create_research_team

        team = create_research_team()

        # 创建任务
        task = team.create_task(
            name="Test Task", description="A test task", required_capabilities=["web_search"]
        )
...
def test_config_validator():
    ...
    try:
        ...
        test_config = {
            "session": {"retention_days": None, "compression_enabled": True},
            "cache": {
                "max_size_mb": 200  # 有效值
            },
        }
        ...
        print(
            f"  [OK] Config Validator: {len(issues)} issues ({errors} errors, {warnings} warnings)"
        )
        return True
...
def test_smart_predictor():
    ...
    try:
        ...
        print(
            f"  [OK] Smart Predictor: {active_alerts} active alerts, prediction available: {pred is not None}"
        )
