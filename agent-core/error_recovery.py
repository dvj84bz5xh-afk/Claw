...
class ErrorRecoveryManager:
    ...
    def attempt_recovery(
        self, record: ErrorRecord, strategy: RecoveryStrategy | None = None
    ) -> RecoveryResult:
        """尝试恢复"""
        with self._lock:
            # 如果没有指定策略，根据错误选择
            ...
            # 执行恢复
            context = {"error": record, "component": record.component, "context": record.context}
    ...
    def generate_report(self) -> str:
        ...
        lines.extend(["-" * 60, "By Component:"])

        for comp, count in summary["by_component"].items():
            lines.append(f"  {comp}: {count}")

        lines.extend(["-" * 60, f"Circuit Breakers: {len(summary['circuit_breakers'])}", "=" * 60])
...
# 装饰器：自动错误恢复
def with_recovery(
    component: str, severity: ErrorSeverity = ErrorSeverity.MEDIUM, auto_recover: bool = True
):
    """自动恢复装饰器"""
