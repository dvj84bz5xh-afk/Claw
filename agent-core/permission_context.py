...
class PermissionContext:
    ...
    # 工具所需的最小权限映射
    TOOL_PERMISSION_MAP: dict[str, PermissionLevel] = field(
    ...
    def can_use_tool(self, tool_name: str) -> tuple[bool, str]:
        ...
        if level_order[self.level] < level_order[required_level]:
            return (
                False,
                f"工具 '{tool_name}' 需要 {required_level.value} 权限，当前为 {self.level.value}",
            )
    ...
    def as_markdown(self) -> str:
        ...
        lines.extend(
            [
                ...
                *(
                    f"- `{tool}` → {level.value}"
                    for tool, level in sorted(self.TOOL_PERMISSION_MAP.items())
                ),
            ]
        )
