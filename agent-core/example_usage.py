...
def demo_session_manager():
    ...
    session = manager.create_session(
        prompt="分析代码仓库", context={"workspace": "/home/user/project", "task": "code_review"}
    )
...
def demo_command_router():
    ...
    router.register_simple(name="my_status", description="显示我的系统状态", handler=status_handler)
...
def demo_integration():
    ...
    session = session_mgr.create_session(
        prompt="分析项目代码", context={"workspace": str(Path.cwd())}
    )
