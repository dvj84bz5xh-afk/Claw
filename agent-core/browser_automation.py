...
class BrowserAutomator:
        ...
        if BEAUTIFULSOUP_AVAILABLE:
            ...
            for element in found_elements:
                ...
                web_element = WebElement(
                    selector=element_selector, content=content_text, attributes=attrs
                )
                elements.append(web_element)

        self._record_operation("extract_elements", {"selector": selector, "count": len(elements)})
    ...
    def take_screenshot(self, filename: str = None, full_page: bool = True) -> str | None:
        """截图"""
        if not self.browser:
            return None
        ...
        try:
            ...
            self._record_operation(
                "take_screenshot", {"path": str(screenshot_path), "full_page": full_page}
            )
    ...
    def execute_script(self, script: str) -> Any | None:
        """执行JavaScript脚本"""
        if not self.browser:
            return None
        ...
        try:
            ...
            self._record_operation(
                "execute_script", {"script": script[:50] + "..." if len(script) > 50 else script}
            )
            return result
