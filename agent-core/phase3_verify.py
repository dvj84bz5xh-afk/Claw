...
try:
    from tool_registry_trait import BashTool, FileReadTool, ToolRegistry
...
try:
    from git_context_enhanced import GitContextProvider
...
try:
    from task_registry import TaskExecutor, TaskPriority, TaskRegistry

    registry = TaskRegistry()

    task = registry.create_task(
        name="Phase3_Test_Task", target=lambda: "Task completed", priority=TaskPriority.HIGH
    )
