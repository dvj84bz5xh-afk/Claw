...
class EnvironmentOptimizer:
    ...
    def run_optimization(self) -> list[OptimizationResult]:
        ...
        for optimizer in self.optimizers:
            ...
            try:
                ...
                status_icon = {"fixed": "[OK]", "already_ok": "[OK]", "failed": "[ERR]"}.get(
                    result.status, "[?]"
                )
    ...
    def generate_report(self, results: list[OptimizationResult]) -> str:
        ...
        for result in results:
            icon = {"fixed": "[OK]", "already_ok": "[OK]", "failed": "[ERR]"}.get(
                result.status, "[?]"
            )
...
def main():
    ...
    # 保存报告
    report_file = (
        workspace_root / "logs" / f"optimization-report-{datetime.now():%Y%m%d-%H%M%S}.txt"
    )
