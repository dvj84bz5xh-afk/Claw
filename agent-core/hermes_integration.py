...
# 导入现有模块
try:
    from memory_enhancement import MemoryItem, MemoryManager, MemoryType
except ImportError:
...
try:
    from multi_solution_generator import MultiSolutionGenerator, SolutionCandidate
except ImportError:
...
class HermesIntegration:
    ...
    def __init__(self):
        ...
        self.communication_bus.register_agent(
            self.agent_id, "HermesIntegration", AgentRole.COORDINATOR
        )
    ...
    def _setup_callbacks(self):
        ...
        # 知识分享回调：存储到记忆
        self.communication_bus.register_callback(
            MessageType.SHARE_KNOWLEDGE, self._handle_share_knowledge
        )
    ...
    def integrate_with_git_system(self, git_integration: GitMultiSolutionIntegration):
        ...
        # 注册Git相关回调
        self.communication_bus.register_callback(
            MessageType.TASK_ASSIGNMENT, lambda msg: git_skill_creator.handle_git_task(msg)
        )
    ...
    def _create_agent(self, agent_config: dict[str, Any]) -> str | None:
        """创建单个智能体"""
        try:
    ...
    def get_integration_report(self) -> dict[str, Any]:
        ...
        return {
            "integration": {"name": "Hermes Integration", "version": "1.0.0", "status": "active"},
            "components": {
                "memory_system": {
                    "status": "enabled"
                    if hasattr(self.memory_manager, "save_memory")
                    else "disabled",
                    "stats": memory_stats,
                },
                "skill_system": {
                    "status": "enabled"
                    if hasattr(self.skill_creator, "create_skill_from_solution")
                    else "disabled",
                    "total_skills": len(skill_files),
                },
                "communication_system": {"status": "enabled", "stats": agent_status},
            },
...
class GitSkillGenerator:
    ...
    def __init__(
        self, skill_creator: SkillAutoCreator, git_integration: GitMultiSolutionIntegration
    ):
    ...
    def _generate_git_solutions(self, message: AgentMessage):
        ...
        try:
            ...
            solutions = self.git_integration.generate_solutions_for_context(
                problem=problem, context=context, num_solutions=3
            )
