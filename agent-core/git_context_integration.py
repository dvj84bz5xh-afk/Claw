...
# 导入TimesFM核心模块
from task_patcher import TaskPatch
...
class GitContext:
    ...
    def has_modified_files(self) -> bool:
        """是否有修改的文件"""
        return any(
            c.change_type in [GitChangeType.MODIFIED, GitChangeType.ADDED]
...
class GitContextProvider:
    ...
    def get_context(self, force_refresh: bool = False) -> GitContext:
        ...
        if not lines:
            ...
            for line in lines:
                ...
                context.file_changes.append(
                    GitFileChange(path=file_path, change_type=change_type, staged=status[0] != " ")
                )

        # 获取最近提交
        stdout, _, _ = self._run_git(["log", "--oneline", "--format=%H|%h|%s|%an|%at", "-10"])
    ...
    def get_detailed_status(self) -> GitStatus:
        ...
        status.modified_files = [
            c.path for c in context.file_changes if c.change_type == GitChangeType.MODIFIED
        ]
        status.staged_files = [c.path for c in context.file_changes if c.staged]
        status.untracked_files = [
            c.path for c in context.file_changes if c.change_type == GitChangeType.UNTRACKED
        ]
    ...
    def _get_ahead_behind_count(self) -> tuple[int, int]:
        ...
        try:
            ...
            # 获取计数
            stdout, _, code = self._run_git(
                ["rev-list", "--left-right", "--count", f"{current_branch}...{upstream}"]
            )
...
class GitContextUncertaintyAnalyzer:
    ...
    def get_relevant_code_context(self, keywords: list[str]) -> dict[str, Any]:
        """获取与关键词相关的代码上下文"""
        git_context = self.git_provider.get_context()
        result = {"relevant_files": [], "relevant_commits": [], "suggested_edits": []}
...
def inject_git_context_to_prompt(prompt: str, git_provider: GitContextProvider) -> str:
    ...
    if git_context.recent_commits:
        ...
        for i, commit in enumerate(git_context.recent_commits[:3], 1):
            ...
            git_context_header += (
                f"  {i}. [{commit.short_hash}] {commit.message[:50]}... ({time_str})\n"
            )
            ...
            **base_result,
            "git_context": {
                ...
                "modified_file_count": len(
                    self.git_provider.get_context().get_modified_file_paths()
                ),
