...
class PerformanceProfiler:
    ...
    def __init__(self):
        self.metrics: dict[str, FunctionMetrics] = defaultdict(lambda: FunctionMetrics(name=""))
        self.enabled = True
    ...
    def get_report(self) -> dict:
        ...
        return {
            "summary": {
                ...
                "avg_time_per_call_ms": round(total_time / total_calls, 2)
                if total_calls > 0
                else 0,
            },
            "slow_functions": [
            ...
            "hotspot_functions": [
                {"name": m.name, "call_count": m.call_count, "avg_time_ms": round(m.avg_time_ms, 2)}
    ...
    def analyze_code(self, code: str, filename: str = "") -> list[dict]:
        ...
        for rule_name, rule in self.optimization_rules.items():
            ...
            if re.search(rule["pattern"], code, re.IGNORECASE):
                suggestions.append(
                    {"rule": rule_name, "suggestion": rule["suggestion"], "severity": "medium"}
                )
