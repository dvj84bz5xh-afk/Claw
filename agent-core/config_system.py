...
class PlanningConfig:
    ...
    planning_strategy: PlanningStrategy = field(default_factory=lambda: PlanningStrategy.ADAPTIVE)
...
class ConfigManager:
    ...
    def __init__(self, default_config: AgentCapabilityConfig | None = None):
    ...
    def rollback(self, steps: int = 1):
        ...
        if steps > len(self._config_history):
            raise ValueError(
                f"Cannot rollback {steps} steps, only {len(self._config_history)} available"
            )
