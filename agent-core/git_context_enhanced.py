...
class GitContext:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "staged_files": [{"path": f.path, "status": f.status} for f in self.staged_files],
...
class GitContextProvider:
    ...
    def _run_git(self, args: list[str], cwd: Path | None = None) -> tuple[str, str, int]:
        """运行 git 命令"""
        try:
    ...
    def get_status(self) -> tuple[GitStatus, str]:
        ...
        if staged and modified:
            return (
                GitStatus.MODIFIED,
                f"已暂存 {sum(1 for l in lines if l[0] != ' ')} 个, 未暂存修改 {sum(1 for l in lines if l[1] != ' ')} 个",
            )
        elif staged:
    ...
    def get_file_changes(self) -> tuple[list[GitFileChange], list[GitFileChange], list[str]]:
        ...
        for line in stdout.strip().split("\n"):
            ...
            if line.startswith("??"):
                ...
                if worktree_status != " ":
                    modified.append(
                        GitFileChange(path=file_path, status=worktree_status, staged=False)
                    )
    ...
    def get_recent_commits(self, count: int = 10) -> list[GitCommit]:
        ...
        stdout, _, code = self._run_git(["log", f"-{count}", f"--pretty=format:{format_str}"])
        ...
        for line in stdout.strip().split("\n"):
            ...
            if commit:
                # 获取统计信息
                stat_stdout, _, _ = self._run_git(["show", "--stat", "--format=", commit.hash])
                if stat_stdout:
                    # 解析统计
        ...
        if code == 0:
            for line in stdout.strip().split("\n"):
                ...
                if "HEAD" not in branch_name:
                    remote_branches.append(GitBranch(name=branch_name, is_remote=True))
    ...
    def get_contributors(self) -> list[str]:
        """获取贡献者列表"""
        stdout, _, code = self._run_git(["log", "--format=%an", "--all", "--no-merges"])
        if code == 0:
            contributors = {}
...
class GitContextInjector:
    ...
    def __init__(self, provider: GitContextProvider):
        ...
        self._last_context: GitContext | None = None
    ...
    def get_context(self, force_refresh: bool = False) -> GitContext:
        ...
        if (
            not force_refresh
            and self._last_context
            and self._last_update
            and now - self._last_update < self._cache_ttl
        ):
