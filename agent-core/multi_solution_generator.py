...
class MultiSolutionGenerator:
    ...
    def rank_solutions(
        self, solutions: list[SolutionCandidate], criteria: RankingCriteria | None = None
    ) -> list[RankedSolution]:
        """
        多维度方案排序
        ...
        for solution in filtered_solutions:
            ...
            weighted_scores = {
                ...
                * weights.get("technical_feasibility", 0.25),
    ...
    def _check_constraints(self, solution: SolutionCandidate, criteria: RankingCriteria) -> bool:
        ...
        resource_order = {
            ...
            ResourceIntensity.MEDIUM: 2,
            ResourceIntensity.HEAVY: 3,
        }
    ...
    def generate_comparison_report(
        ...
        ranked_solutions: list[RankedSolution] | None = None,
    ) -> dict[str, Any]:
        """生成方案比较报告"""
        ...
        for solution in solutions:
            stats["solution_types"][solution.type.value] = (
                stats["solution_types"].get(solution.type.value, 0) + 1
            )
            stats["risk_distribution"][solution.risk_level.value] = (
                stats["risk_distribution"].get(solution.risk_level.value, 0) + 1
            )
            stats["resource_distribution"][solution.resource_intensity.value] = (
                stats["resource_distribution"].get(solution.resource_intensity.value, 0) + 1
            )
...
if __name__ == "__main__":
    ...
    solutions = generator.generate_solutions(
        problem=test_problem, context=context, num_solutions=4, diversity_threshold=0.6
    )
