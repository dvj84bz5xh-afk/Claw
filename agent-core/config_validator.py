...
DEFAULT_SCHEMAS = [
    ...
    ConfigSchema(
        "session.auto_cleanup", bool, required=False, default=False, description="是否自动清理"
    ),
    ConfigSchema(
    ...
    ConfigSchema(
        "logging.enable_audit", bool, required=False, default=True, description="启用审计日志"
    ),
    # Security 配置
    ConfigSchema(
...
class ConfigValidator:
    ...
    def __init__(self, schemas: list[ConfigSchema] | None = None):
    ...
    def generate_report(self, issues: list[ValidationIssue] | None = None) -> str:
        """生成验证报告"""
    ...
    def is_valid(self, issues: list[ValidationIssue] | None = None) -> bool:
        """检查是否有效（无ERROR及以上）"""
        issues = issues or self.issues
        return not any(i.level in [ValidationLevel.ERROR, ValidationLevel.CRITICAL] for i in issues)
...
class ConfigDiagnostics:
    ...
    def _find_config_file(self) -> str | None:
        """查找配置文件"""
        candidates = [".workbuddy/workspace.json", ".workbuddy/config.json", "config.json"]
    ...
    def _detect_changes(self, original: dict, fixed: dict) -> list[dict]:
        ...
        def compare(path: str, orig: Any, new: Any):
            if isinstance(orig, dict) and isinstance(new, dict):
                ...
                changes.append({"path": path, "from": orig, "to": new})
