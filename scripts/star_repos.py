...
tokens_found = []
for key in ['GITHUB_TOKEN', 'GH_TOKEN', 'GITHUB_API_KEY', 'OATH_TOKEN']:
...
if not tokens_found:
    ...
    for k in os.environ.keys():
        if 'token' in k.lower():
            print(f"    - {k}")
    if not any('token' in k.lower() for k in os.environ.keys()):
        print("    （无）")
else:
    ...
    for repo in repos:
        ...
        try:
            ...
            if r.status_code in [204, 201]:
                ...
                print(f"  ✅ {repo} - 已star（之前）")
            else:
                print(f"  ❌ {repo} - 失败({r.status_code}): {r.text[:100]}")
        except Exception as e:
            print(f"  ❌ {repo} - 错误: {e}")
else:
    print("\n⚠️ 无GitHub Token，无法自动star。")
    print("手动star链接:")
