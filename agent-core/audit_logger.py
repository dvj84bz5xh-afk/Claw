...
class AuditLogger:
    ...
    def __init__(self, storage_dir: str | None = None, max_memory_records: int = 10000):
    ...
    def log_file_access(
        self, file_path: str, operation: str, actor: str = "system", session_id: str | None = None
    ):
        """记录文件访问"""
        return self.log(
            level=AuditLevel.ACTION,
    ...
    def log_command(
        self, command: str, exit_code: int, actor: str = "system", session_id: str | None = None
    ):
        """记录命令执行"""
        return self.log(
            level=AuditLevel.ACTION,
    ...
    def generate_report(self, hours: int = 24) -> str:
        ...
        for record in records:
            ...
            stats["by_category"][record.category.value] = (
                stats["by_category"].get(record.category.value, 0) + 1
            )
...
class AuditContext:
    ...
    def __init__(
        self, logger: AuditLogger, action: str, actor: str = "system", session_id: str | None = None
    ):
        self.logger = logger
        self.action = action
        self.actor = actor
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            self.logger.error(
                ...
                actor=self.actor,
                session_id=self.session_id,
                details={"duration_ms": duration, "error": str(exc_val)},
            )
        else:
            self.logger.info(
                AuditCategory.SYSTEM,
