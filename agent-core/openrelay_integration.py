...
class OpenRelayIntegration:
    ...
    def __init__(self, config: OpenRelayConfig | None = None, workspace_path: str | None = None):
        ...
        # 创建数据目录
        self.data_dir = Path(
            self.config.data_dir or (self.workspace_path / ".workbuddy" / "openrelay")
        )
    ...
    async def route_request(
        self, tool_name: str, request_data: dict[str, Any], preferred_provider: str | None = None
    ) -> dict[str, Any]:
        """
        路由AI请求到最佳提供商
        ...
        try:
            ...
            response = await self.client.post(
                provider_url, json=request_data, headers={"Content-Type": "application/json"}
            )
    ...
    async def _select_best_provider(
        self, tool_name: str, preferred_provider: str | None = None
    ) -> AIProvider | None:
        """选择最佳AI提供商"""
        ...
        # 过滤启用的提供商
        available_providers = [
            p for p in self.config.providers if p.enabled and p.status == ProviderStatus.ACTIVE
        ]
    ...
    async def _fallback_request(
        self, tool_name: str, request_data: dict[str, Any], failed_provider_id: str
    ) -> dict[str, Any]:
        """故障转移请求到备用提供商"""
        # 排除失败的提供商
        backup_providers = [
...
async def main():
    ...
    # 创建集成管理器
    integration = OpenRelayIntegration(workspace_path=r"c:\Users\10127\WorkBuddy\Claw")
