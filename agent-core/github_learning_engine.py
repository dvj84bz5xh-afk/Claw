...
class LearnedInsight:
    ...
    code_snippet: str | None = None  # 代码片段
...
class ImprovementSuggestion:
    ...
    reference_code: str | None = None  # 参考代码
    ...
    def analyze_project(
        ...
        for suggestion in suggestions:
            if suggestion.priority == Priority.P0:
                ...
                action_items.append(f"[P1] 规划: {suggestion.title}")
    ...
    def get_learning_stats(self) -> dict:
        ...
        return {
            ...
            "p0_pending": len(
                [
                    p
                    for p in self.progress["pending_implementations"]
                    if p["priority"] == "P0" and p["status"] == "pending"
                ]
            ),
        }

    def get_pending_implementations(self, priority: Priority | None = None) -> list:
        """获取待实施项"""
        pending = [p for p in self.progress["pending_implementations"] if p["status"] == "pending"]
...
class AgentRuntime:
    async def send_message(self, message: Message, recipient: AgentId):
        await self._message_queue.put((message, recipient))
            ...
            LearnedInsight(
                category="innovation", description="GroupChat机制实现多Agent协作，支持多种选择策略"
            ),
        ]
    ...
    def create_mcp_insights() -> list[LearnedInsight]:
        ...
        return [
            LearnedInsight(
                category="architecture", description="标准化的工具协议，分离工具提供者和使用者"
            ),
            LearnedInsight(
                category="mechanism",
...
class Server:
    ...
    async def handle_call_tool(name: str, arguments: dict):
        return await tool_registry.execute(name, arguments)
            ...
            LearnedInsight(category="innovation", description="声明式工具定义，自动生成功能描述"),
        ]
