...
class ProbabilisticOutput:
    ...
    # 置信区间
    confidence_interval: tuple[Any, Any] | None = None
    ...
    def get_quantile(self, q: float) -> Any | None:
        """获取指定分位数的预测值"""
    ...
    def get_median(self) -> Any | None:
        """获取中位数预测 (0.5分位数)"""
        return self.get_quantile(0.5)

    def get_prediction_range(self) -> tuple[Any, Any] | None:
        """获取预测范围 (10%-90%分位数)"""
