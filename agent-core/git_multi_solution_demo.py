...
from git_multi_solution_integration import GitContextAwareSolutionGenerator
...
class GitMultiSolutionDemo:
    ...
    def demo_strategy_selection(self):
        ...
        test_scenarios = [
            ...
            },
            {
                "name": "功能开发中",
                "status": self._create_mock_status(
                    current_branch="feature/new-module", ahead_count=5
                ),
            },
            {
                "name": "发布准备",
    ...
    def _create_mock_status(self, **kwargs):
        ...
        mock_status = GitStatus(
            ...
            has_uncommitted_changes=kwargs.get(
                "has_uncommitted_changes", real_status.has_uncommitted_changes
            ),
        )

        # 处理数量参数
        if "modified_count" in kwargs:
            mock_status.modified_files = [
                f"test_file_{i}.py" for i in range(kwargs["modified_count"])
            ]

        if "untracked_count" in kwargs:
            mock_status.untracked_files = [
                f"untracked_{i}.tmp" for i in range(kwargs["untracked_count"])
            ]
    ...
    def demo_solution_generation(self):
        ...
        for i, problem in enumerate(test_problems, 1):
            ...
            solutions = self.integrated_gen.generate_solutions_for_context(
                problem=problem["description"], context=problem["context"], num_solutions=3
            )
        ...
        if solutions:
            ...
            if len(solutions) > 1:
                ...
                for i, solution in enumerate(solutions[1:4], 2):
                    print(
                        f"  {i}. {solution.id[:8]} - 评分: {solution.total_score_with_git:.2f} "
