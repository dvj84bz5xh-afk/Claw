...
class HealthChecker:
    ...
    def _check_agent_core_modules(self):
        ...
        if enhanced_existing:
            ...
            self._add_check(
                HealthCheckItem(
                    ...
                    category="核心模块",
                    status=CheckStatus.WARNING,
                    message="基础核心模块存在，建议升级到增强版",
                    details={
                        "legacy_modules": legacy_existing,
                        "missing_enhanced": [
                            m for m in enhanced_modules if m not in enhanced_existing
                        ],
                    },
        ...
        # 检查新增强模块
        enhanced_modules = ["permission_enhanced.py", "session_compaction.py", "health_check.py"]
        ...
        self._add_check(
            HealthCheckItem(
                ...
                category="核心模块",
                status=CheckStatus.OK
                if len(enhanced_existing) == len(enhanced_modules)
        ...
        try:
            ...
            self._add_check(
                HealthCheckItem(
                    ...
                    details={"storage_path": str(session_path), "session_count": len(sessions)},
                )
            )
        except Exception as e:
            self._add_check(
    ...
    def _check_git_config(self):
        ...
        try:
            result = subprocess.run(
                ["git", "config", "--global", "--list"], capture_output=True, text=True, timeout=5
            )
    ...
    def _check_session_compaction(self):
        ...
        try:
            ...
            from session_compaction import SessionManager, TokenEstimator
        ...
        if not missing:
            ...
            self._add_check(
                HealthCheckItem(
                    ...
                    status=CheckStatus.OK,
                    message=msg,
                    details={"existing": existing, "optional_missing": optional_missing},
                )
            )
        else:
            self._add_check(
...
# 便捷函数
def run_health_check(workspace_root: Path | None = None) -> HealthReport:
    """运行健康检查"""
