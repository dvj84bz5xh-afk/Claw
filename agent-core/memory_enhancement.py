...
class MemoryItem:
    ...
    session_id: str | None = None  # 所属会话ID
    ...
    def calculate_importance(self, access_count: int = 1) -> float:
        ...
        self.importance_score = (
            base_score * 0.25 + type_weight * 0.35 + recency_factor * 0.25 + frequency_factor * 0.15
        )
...
class MemoryManager:
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        # 计算平均重要性
        avg_importance = (
            sum(m.importance_score for m in self.memory_index.values()) / total if total > 0 else 0
        )
    ...
    def _load_from_disk(self, memory_file: Path) -> MemoryItem | None:
        """从磁盘加载记忆"""
        try:
...
class MemoryUtils:
    ...
    def extract_memory_from_text(
        text: str, memory_type: MemoryType = MemoryType.FACT, auto_tags: bool = True
    ) -> MemoryItem:
        """从文本中提取记忆"""
        # 自动生成标签（简单实现）
        tags = []
    ...
    @staticmethod
    def create_solution_memory(
        solution_content: str, solution_type: str, tags: list[str]
    ) -> MemoryItem:
        """创建解决方案记忆"""
        memory = MemoryItem(
            ...
            metadata={"solution_type": solution_type, "created_by": "multi_solution_generator"},
        )
        return memory

    @staticmethod
    def create_skill_memory(
        skill_name: str, skill_description: str, triggers: list[str], implementation: str
    ) -> MemoryItem:
        """创建技能记忆"""
        memory = MemoryItem(
